...
export const invoice = generateInvoice();
...
export async function createInvoicePDF(invoice) {
    ...
    //   await doc.addImage(imgData, format, OFFSET, 0, WIDTH, HEIGHT);
    doc.addImage(imgDataUrl, 'PNG', OFFSET, 0, WIDTH, HEIGHT);
    const bgColor = [40, 31, 51];
    doc.setFontSize(15);
    doc.setTextColor("red");
    doc.text(`EASY GOODS AND SERVICES`, WIDTH + 5 + OFFSET, HEIGHT - 1.5, {
        renderingMode: "stroke",
    });
    doc.setFontSize(30);
    doc.setTextColor("red");
    doc.text(`INVOICE`, doc.internal.pageSize.width - OFFSET, HEIGHT - 1 + 10, {
        align: "right",
    });
    ...
    doc.autoTable({
        ...
        didDrawPage: (data) => {
            ...
            if (data.page !== totalPages) {
                ...
                doc.text(`email: rosemary@gmail.com`, doc.internal.pageSize.width - OFFSET, doc.internal.pageSize.height - 5, { align: "right" });
            }
            currentPage++;
        },
    });
    ...
    doc.setFontSize(10);
    doc.setTextColor("red");
    doc.text(`USER INFORMATION`, OFFSET, doc.autoTable.previous.finalY + 20, { align: "left" });
    doc.setFontSize(10);
    doc.setTextColor("black");
    doc.text(`FullName: Rose Mary`, OFFSET, doc.autoTable.previous.finalY + 20 + 4, { align: "left" });
    doc.text(`Phone: 670000002`, OFFSET, doc.autoTable.previous.finalY + 20 + 4 + 4, { align: "left" });
    doc.text(`email Address: rosemary@gmail.com`, OFFSET, doc.autoTable.previous.finalY + 20 + 4 + 4 + 4, { align: "left" });
    doc.addImage(imgData, format, OFFSET, doc.autoTable.previous.finalY + 20 + 4 + 4 + 4 + 4, WIDTH + 5, HEIGHT + 5);
    doc.setTextColor("red");
    doc.text(`PAYMENT INFORMATION`, doc.internal.pageSize.width - OFFSET, doc.autoTable.previous.finalY + 20, { align: "right" });
    doc.setTextColor("black");
    doc.text(`FullName: Fred Morgan`, doc.internal.pageSize.width - OFFSET, doc.autoTable.previous.finalY + 20 + 4, { align: "right" });
    doc.text(`FullName: Fred Morgan`, doc.internal.pageSize.width - OFFSET, doc.autoTable.previous.finalY + 20 + 4 + 5, { align: "right" });
    // Save the PDF
