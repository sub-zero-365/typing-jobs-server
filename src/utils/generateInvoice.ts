...
//   await doc.addImage(imgData, format, OFFSET, 0, WIDTH, HEIGHT);
  ...
  doc.text(
    `FullName: Rose Mary`,
    OFFSET,
    (doc as any).autoTable.previous.finalY + 20 + 4,
    { align: "left" }
  );
  doc.text(
    `Phone: 670000002`,
    OFFSET,
    (doc as any).autoTable.previous.finalY + 20 + 4 + 4,
    { align: "left" }
  );
  doc.text(
    `email Address: rosemary@gmail.com`,
    OFFSET,
    (doc as any).autoTable.previous.finalY + 20 + 4 + 4 + 4,
    { align: "left" }
  );
  doc.addImage(
    imgData,
    format,
    OFFSET,
    (doc as any).autoTable.previous.finalY + 20 + 4 + 4 + 4 + 4,
    WIDTH + 5,
    HEIGHT + 5
  );
  doc.setTextColor("red");
  ...
  doc.text(
    `FullName: Fred Morgan`,
    doc.internal.pageSize.width - OFFSET,
    (doc as any).autoTable.previous.finalY + 20 + 4 + 5,
    { align: "right" }
  );
  // Save the PDF
